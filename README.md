...
**기능**: 일/주/월/연봉 4개 타임프레임 기술적 지표(이동평균·RSI·MACD·볼린저·스토캐스틱·ATR·지지저항) · 결정론적 방향성 예측(단기 1~4주·중기 1~3개월 시나리오 확률·신뢰도) · 뉴스 호재/악재 정리 · 목표가/손절가 계산(percent·ATR·지지저항 3방식 동시 제공). 모든 답변은 출력 전 검증 AI 게이트를 통과한다.
...
지표·목표가에 더해 방향성 예측(시나리오 확률)도 제공하지만, 결정론적 점수를 확률·신뢰도로
표현한 참고 정보일 뿐 확정된 미래가 아니며 매매 권유도 아니다. 모든 답변은 결정론적
정합성 검사(`verify.py`)와 별도 검증 에이전트를 거친 뒤에만 출력된다. 투자 판단과 결과는
본인 책임이다.
